from numpy import array, concatenate, zeros, set_printoptions
...
def increase_graph_size_fivefold(graph):
    graph = concatenate((graph, graph+1, graph+2, graph+3, graph+4), axis=1)
    graph[graph == 10] = 1
    graph[graph == 11] = 2
    graph[graph == 12] = 3
    graph[graph == 13] = 4

    graph = concatenate((graph, graph+1, graph+2, graph+3, graph+4), axis=0)
    graph[graph == 10] = 1
    graph[graph == 11] = 2
    graph[graph == 12] = 3
    graph[graph == 13] = 4
...
with open("day15.txt", "r") as file:
    ...
    graph = increase_graph_size_fivefold(graph)
